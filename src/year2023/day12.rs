use std::{fs::File, io, fmt::{self, Display}};
...
impl Display for State {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        write!(f, "pos: {}, remaining: {}, pattern_pos: {}", self.pos, self.remaining, self.pattern_pos)
    }
}
...
fn solve(mut state: State, info: &StaticInfo) -> usize {
    ...
    let mut sum = if match_damage_block(pos, &state.map, info.damaged_pattern[state.pattern_pos] as usize) {
...
fn riddle_2(lines: io::Lines<io::BufReader<File>>) -> String {
    ...
    for l in lines {
        let line = l.unwrap();
        let parts: Vec<&str> = line.trim().split(' ').collect();
        let fifth_map = parts[0].as_bytes().to_vec();
        let fifth_pattern: Vec<u32> = parts[1].split(',').map(|s| s.parse().unwrap()).collect();
        let mut map = fifth_map.clone();
        let mut pattern = fifth_pattern.clone();
        for _ in 0..4 {
            map.push(b'?');
            map.extend(&fifth_map);
            pattern.extend(&fifth_pattern);
        }
