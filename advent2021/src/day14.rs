...
fn read_input(lines: io::Lines<io::BufReader<File>>) ->(String, HashMap<String, String>) {
    ...
    let rules: HashMap<String, String> = lines.skip(1)
        .map(|x| {
...
fn calc_new_polymere(polymere: String, rules: &HashMap<String, String>) -> String {
    ...
    for i in 0..elements.len()-1 {
        ...
        let insertion = rules[&String::from_utf8(elements[i..=i+1].to_vec()).unwrap()].bytes().next().unwrap();
...
fn read_as_vec(lines: io::Lines<io::BufReader<File>>) ->(Vec<u8>, HashMap<(u8,u8), u8>) {
    ...
    let rules: HashMap<(u8, u8), u8> = lines.skip(1)
        .map(|x| {
            let s: Vec<String> = x.split(" -> ").into_iter().map(|x| x.to_owned() ).collect();
            let mut b0 = s[0].bytes();
            ((b0.next().unwrap(), b0.next().unwrap()), s[1].bytes().next().unwrap())    
    }).collect();
    (polymere, rules)
}
...
fn update_score(score: &HashMap<(u8,u8), usize>, rules: &HashMap<(u8,u8), u8>) -> HashMap<(u8,u8), usize> {
