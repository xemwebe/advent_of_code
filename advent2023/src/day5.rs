    ...
    for r in map {
        ...
        for v in &mut orig_values {
            ...
            if v.start<r.start && v.end>r.end {
                new_values.push(ValueRange{ start: r.dest, end: dest_end });
                extra = Some(ValueRange { start: r.end, end: v.end });
                v.end = r.start;
                break;
...
pub fn riddle_5_2(lines: io::Lines<io::BufReader<File>>) {
    ...
    for line in lines {
        ...
        if values.is_empty() {
            ...
            let vals: Vec<u64> = val_str[1].split(' ').filter_map(|s| s.parse().ok() ).collect();
